...
int hh = 0, mm = 0, ss = 0, dd = 0, bb = 0, yy = 0, set_day;    
String Day = "  ";                                          
...
void loop(){  
  ...
  if(mode==1){lcd.setCursor(0,1); lcd.write(2);}
...
if(digitalRead (bt_time) == 1 && digitalRead (bt_alarm) == 1){flag=0;}
 ...
 if(digitalRead (bt_up) == 0){                          
            if (setAlarm<2 && setMode==1)dd=dd+1; 
            if (setAlarm<2 && setMode==2)bb=bb+1;
            if (setAlarm<2 && setMode==3)yy=yy+1;            
            if (setAlarm<2 && setMode==4)hh=hh+1;
            if (setAlarm<2 && setMode==5)mm=mm+1;
            if (setAlarm<2 && setMode==6)ss=ss+1;
            if (setAlarm<2 && setMode==7)set_day=set_day+1;
            //Alarm
